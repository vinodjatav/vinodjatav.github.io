...
const MyLearningsSubPage = (props: PropTypes) => {
    const {buttonRef, data} = props;
    return (
        <Box ref={buttonRef} display="flex" flexDirection="column" sx={{ padding: "80px", gap: "24px" }}>
            <Typography variant='h2' sx={{ fontSize: "60px", lineHeight: "73px" }}>My learnings</Typography>
            <List>
                {data?.map((item) => (
...
export default MyLearningsSubPage